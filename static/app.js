(() => {
  ...
  var graphState = { nodes: [], edges: [], byId: {} };
  ...
  var graphView = { scale: 1, x: 0, y: 0, dragging: false, sx: 0, sy: 0, ox: 0, oy: 0 };
  ...
  var constellationScene = { frame: 0, nodes: [], edges: [], byId: {}, stars: [], ...CONSTELLATION_DEFAULT_CAMERA, paused: false, mode: "rotate", visualiserMode: localStorage.getItem(VISUALISER_MODE_KEY) || "constellation", lastFrameTime: 0, lastInteraction: 0, hits: [], data: null, drag: null, pointers: /* @__PURE__ */ new Map() };
  ...
  function isCanvasVisualiserActive() {
  ...
  function bindReviewControls(queues) {
    $$("#review .review-check").forEach((chk) => chk.onchange = (e) => {
      e.stopPropagation();
  ...
  function renderSelectedReviewQueue(data, append = false) {
    ...
    updateReviewBulkBar();
    $("#reviewQueueSelect").onchange = (e) => {
      selectedReviewQueue = e.target.value;
      reviewOffset = 0;
  ...
  function constellationInspectorDefault() {
    ...
    $("#constellationInspector").innerHTML = neural ? `<div class="inspector-kicker">Neural inspector</div><h3>Nothing selected</h3><p class="muted">Pick a neuron hub, memory soma, or synapse to inspect the underlying read-only source.</p>` : `<div class="inspector-kicker">Constellation inspector</div><h3>Nothing selected</h3><p class="muted">Pick a star, memory, or link to inspect the underlying read-only source.</p>`;
  }
  function inspectConstellationNode(node) {
    ...
    $("#constellationSearch").onclick = () => {
      $("#memoryQuery").value = node.label.replace(/^memory:/, "");
      switchTab("memories");
    };
  }
  ...
  function drawVisualiserFrame(t = 0) {
    if (!isCanvasVisualiserActive()) {
      stopCanvasVisualiserLoop();
      return;
    }
    const mode = constellationScene.visualiserMode === "neural" ? "neural" : "constellation";
    const interval = 16;
  ...
  function inspectNode(node) {
    ...
    $("#graphFilterTriples").onclick = () => {
      $("#tripleQuery").value = node.label;
      switchTab("triples");
    };
    $("#graphSearchMemories").onclick = () => {
      $("#memoryQuery").value = node.label;
      switchTab("memories");
    };
  ...
  function inspectEdge(edge) {
    ...
    $("#edgeTriples").onclick = () => {
      $("#tripleQuery").value = `${edge.subject} ${edge.predicate} ${edge.object}`;
      switchTab("triples");
    };
  }
  ...
  function bindGraphPanZoom() {
    ...
    svg.addEventListener("wheel", (e) => {
      ...
      const px = (e.clientX - rect.left) / rect.width * 1e3;
  ...
  function drawGraph(g) {
    ...
    const vp = $("#graphViewport");
    const w = 1e3, h = 650, cx = w / 2, cy = h / 2, r = 260;
    const nodes = g.nodes.slice(0, 160).map((n, i, a) => ({ ...n, x: cx + Math.cos(i / a.length * Math.PI * 2) * r * (0.65 + i % 5 / 10), y: cy + Math.sin(i / a.length * Math.PI * 2) * r * (0.65 + i % 7 / 14) }));
    const byId = Object.fromEntries(nodes.map((n) => [n.id, n]));
    graphState.nodes = nodes;
    graphState.byId = byId;
  ...
  function inspectThreeNode(node) {
    ...
    $("#threeSearch").onclick = () => {
      $("#memoryQuery").value = String(node.label || "").replace(/^memory:/, "");
      switchTab("memories");
    };
  }
  ...
  function palaceSearchBeacon() {
    ...
    if (!q) {
      $("#palaceHudStatus").textContent = "type a search to place beacon";
      return;
    }
  ...
  function bindPalaceControls() {
    const viewport = $("#palaceViewport");
    if (!viewport || viewport.dataset.controlsBound === "true") return;
    viewport.dataset.controlsBound = "true";
    window.addEventListener("keydown", (e) => {
      if (sectionFor(currentRoute.tab) === "memoryPalace") palaceKeys[e.key.length === 1 ? e.key.toLowerCase() : e.key] = true;
    });
  ...
  function palaceFpsAddRelic(THREE, scene, node, colors) {
    ...
    scene.add(plinth);
    const color = node.contaminated ? 16732039 : cssHexToInt(colors.memory);
  ...
  async function renderMemoryPalace(data) {
    ...
    try {
      ...
      $("#palaceLabels").innerHTML = `<div class="three-fallback-card"><h3>Mnemosyne Labyrinth unavailable</h3><p>This browser could not start WebGL.</p></div>`;
      return;
    }
