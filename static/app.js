...
function memoryItem(item, opts={}){ const role = roleOf(item.content); const roleBadge = role ? `<span class="chip chip-role-${role}">${role}</span>` : ''; const selectedSet = opts.selectedSet || bulkSelection; const checkClass = opts.checkClass || 'memory-check'; const selectable = opts.selectable ? `<label class="memory-select" title="Select memory"><input type="checkbox" class="${esc(checkClass)}" data-id="${esc(item.id)}" ${selectedSet.has(item.id) ? 'checked' : ''} /></label>` : ''; const [liveClass, liveLabel, liveBadgeClass] = liveEventMeta(item); const liveBadge = liveLabel ? `<span class="chip chip-live-${esc(liveClass)}">${esc(liveLabel)}</span>` : ''; const displayContent = cleanContent(item.content); return `<div class="item memory-card ${role ? 'has-role' : ''} ${opts.selectable ? 'selectable' : ''} ${liveClass ? `live-${esc(liveClass)}` : ''}" data-id="${esc(item.id)}">${selectable}<div class="item-topline">${roleBadge}${liveBadge}</div>${meta(item)}<div class="content">${esc(displayContent)}</div></div>`; }
