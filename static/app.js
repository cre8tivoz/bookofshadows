...
function drawConstellation(data){
  ...
  nodes.forEach((n,i) => { const g=document.createElementNS('http://www.w3.org/2000/svg','g'); g.setAttribute('class',`constellation-node ${n.kind || 'entity'}`); g.dataset.id=n.id; g.style.setProperty('--float-x', `${((i%5)-2)*.75}px`); g.style.setProperty('--float-y', `${((i%7)-3)*.7}px`); g.style.setProperty('--float-delay', `${-(i%11)*.7}s`); g.style.setProperty('--pulse-delay', `${-(i%13)*.45}s`); g.onclick = () => inspectConstellationNode(n); const r=Math.min(24, 5 + Math.sqrt(Number(n.weight || n.count || 1))*4); const c=document.createElementNS('http://www.w3.org/2000/svg','circle'); c.setAttribute('cx',n.x); c.setAttribute('cy',n.y); c.setAttribute('r',r); g.appendChild(c); const text=document.createElementNS('http://www.w3.org/2000/svg','text'); text.textContent=(n.label || '').replace(/^memory:/,'mem '); if(text.textContent.length>26) text.textContent=text.textContent.slice(0,23)+'…'; text.setAttribute('x',n.x+r+5); text.setAttribute('y',n.y+4); g.appendChild(text); svg.appendChild(g); });
